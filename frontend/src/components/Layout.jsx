...
const PAGES = {
  '/': 'Dashboard',
  '/minhas-ferias': 'Minhas Ferias',
  '/solicitar': 'Solicitar Ferias',
...
function LinkItem({ to, end, icon, children }) {
  return (
    ...
    <div className="layout">
      <aside className="sidebar">
        <div className="sidebar-logo">
          ...
          <span className="logo-text">Ferias<span>RH</span></span>
        </div>
        ...
        <nav className="sidebar-nav">
          <div className="nav-group">
            ...
            <LinkItem to="/minhas-ferias" icon={Icon.calendar}>Minhas Ferias</LinkItem>
            <LinkItem to="/solicitar" icon={Icon.plus}>Solicitar Ferias</LinkItem>
            <LinkItem to="/disponibilidade" icon={Icon.grid}>Disponibilidade</LinkItem>
          </div>

          {user?.role === 'admin' && (
            <div className="nav-group">
              <span className="nav-label">Administracao</span>
              <LinkItem to="/usuarios" icon={Icon.users}>Usuarios</LinkItem>
              <LinkItem to="/relatorios" icon={Icon.chart}>Relatorios</LinkItem>
      ...
      <div className="main-area">
        <header className="topbar">
          <div>
            <div className="topbar-title">{PAGES[location.pathname] ?? 'FeriasRH'}</div>
            <div className="topbar-sub">Sistema de Gestao de Ferias ONRTDPJ</div>
          </div>

          <div className="topbar-right" ref={dropRef}>
            <button className="user-pill" onClick={() => setDropOpen(open => !open)}>
              <div className="avatar">{user?.nome?.[0]?.toUpperCase() || 'U'}</div>
              <div>
